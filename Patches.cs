...
internal static class GrabbableObjectControlTipsPatch
{
    ...
    private static void Postfix(GrabbableObject __instance)
    {
        ...
        // Q = ItemInteractLeftRight(false), same slot spray paint uses for Shake.
        var q = InputUtil.TipLabel(UnityEngine.KeyCode.Q);
        var r = InputUtil.TipLabel(Plugin.RefillKey.Value);
        hud.ChangeControlTip(2, Plugin.AllowKeyRefill.Value
            ? $"Refill : [{q}] / [{r}]"
            : $"Refill : [{q}]");
    }
}
